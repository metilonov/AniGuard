...
def test_case_reel_uses_stable_mobile_webview_layout() -> None:
    html = (STATIC / "index.html").read_text(encoding="utf-8")
    # v2 intentionally removed the rigid 9:16 sheet: Telegram WebView
    # now uses a bounded bottom sheet and fixed reel cells.
    assert "#case-opening-dialog.case-opening-dialog" in html
    assert "width:min(438px,100%)" in html
    assert "max-height:calc(100dvh - 16px)" in html
    assert "min-width:clamp(78px,23vw,96px)!important" in html
