...
`BOT_ID` вручную не публикуйте в репозитории. Приложение читает его из окружения BotHost. Админ-панель и показатели ресурсов обновляются каждую секунду. История записывается раз в 10 секунд, чтобы не раздувать базу данных.
