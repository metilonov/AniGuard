...
class AccessMiddleware(BaseMiddleware):
    async def __call__(
        ...
        raw_message_text = (getattr(message_obj, "text", None) or "") if message_obj else ""
        command_token = (((raw_message_text.split(maxsplit=1) or [""])[0]).split("@", 1)[0].lower())
        ...
        is_registration_callback = callback_data.startswith("register_chat:")

        # Пока новая группа не прошла ручную регистрацию, обычные
        # команды и автомодерация в ней не запускаются.
        if chat_obj and chat_obj.type in {ChatType.GROUP, ChatType.SUPERGROUP}:
            async with SessionFactory() as registration_session:
                registration_chat = await registration_session.get(Chat, chat_obj.id)
                registration_complete = bool(
                    registration_chat
                    and registration_chat.is_active
                    and (registration_chat.settings or {}).get("registration_completed", True)
                )
...
async def bot_membership_changed(event: ChatMemberUpdated) -> None:
    """Prepare a Telegram group and wait for explicit AniGuard registration."""
    ...
    async with SessionFactory() as session:
        stored_chat = await session.get(Chat, event.chat.id)
        if status_value in {"left", "kicked"}:
            if stored_chat is not None:
                stored_chat.is_active = False
                stored_settings = dict(stored_chat.settings or {})
                stored_settings["registration_completed"] = False
        ...
        already_registered = bool(
            ...
            and (stored_chat.settings or {}).get("registration_completed", True)
        )
        if already_registered:
            try:
                await sync_chat_from_telegram(session, bot, event.chat.id)
            except Exception as exc:
                logger.warning("Could not refresh registered chat %s: %s", event.chat.id, exc)
            await session.commit()
            return

        pending_chat = await upsert_chat(session, event.chat)
        pending_chat.is_active = False
        pending_settings = dict(pending_chat.settings or {})
        pending_settings.update({
            "registration_completed": False,
            "registration_requested_at": utcnow().isoformat(),
        })
    ...
    if should_send_prompt:
        try:
            await send_registration_prompt(event.chat.id, event.chat.title or "Новая беседа")
        except Exception as exc:
            logger.exception("Could not send registration prompt to chat %s: %s", event.chat.id, exc)


REQUIRED_BOT_ADMIN_RIGHTS: tuple[tuple[str, str], ...] = (
    ("can_delete_messages", "Удаление сообщений"),
    ("can_restrict_members", "Блокировка и ограничение участников"),
...
def registration_keyboard(chat_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="✅ Зарегистрировать беседу", callback_data=f"register_chat:{chat_id}")
    ]])


def missing_registration_rights(bot_member: Any) -> list[str]:
    status_value = getattr(bot_member.status, "value", str(bot_member.status))
    ...
    return [title for attribute, title in REQUIRED_BOT_ADMIN_RIGHTS if not bool(getattr(bot_member, attribute, False))]


async def send_registration_prompt(chat_id: int, title: str) -> None:
    safe_title = html.escape(title or "Новая беседа")
    await bot.send_message(
        chat_id=chat_id,
        text=(
            "⚠️ <b>Беседа не зарегистрирована</b>\n\n"
            f"Группа: <b>{safe_title}</b>\n\n"
            "AniGuard добавлен в эту беседу, но его функции пока отключены.\n\n"
            "Для запуска защиты владелец или администратор группы должен нажать кнопку регистрации.\n\n"
            "После нажатия AniGuard проверит свой статус и необходимые права."
        ),
        reply_markup=registration_keyboard(chat_id),
    )


@router.callback_query(F.data.startswith("register_chat:"))
async def register_chat_callback(callback: CallbackQuery) -> None:
    if not callback.data or callback.message is None:
        await callback.answer("Некорректные данные регистрации.", show_alert=True)
        return
    try:
        _, raw_chat_id = callback.data.split(":", 1)
        chat_id = int(raw_chat_id)
    except (TypeError, ValueError):
        await callback.answer("Некорректный идентификатор беседы.", show_alert=True)
        return

    message_chat = callback.message.chat
    if message_chat.id != chat_id or message_chat.type not in {ChatType.GROUP, ChatType.SUPERGROUP}:
        await callback.answer("Эта кнопка относится к другой беседе.", show_alert=True)
        return

    try:
        actor_member = await bot.get_chat_member(chat_id, callback.from_user.id)
        actor_status = getattr(actor_member.status, "value", str(actor_member.status))
        if actor_status not in {"creator", "administrator"}:
            await callback.answer("Регистрацию может выполнить только владелец или администратор.", show_alert=True)
            return
        ...
        async with SessionFactory() as session:
            ...
            maintenance = bool(system_row and isinstance(system_row.value, dict) and system_row.value.get("maintenance"))
            if maintenance:
                await callback.answer("AniGuard временно находится на техническом обслуживании.", show_alert=True)
                return
            if await get_block_record(session, "user", callback.from_user.id):
                await callback.answer("Ваш доступ к AniGuard заблокирован.", show_alert=True)
                return
            if await get_block_record(session, "chat", chat_id):
                await callback.answer("Эта беседа заблокирована в AniGuard.", show_alert=True)
                return

        bot_info = await bot.get_me()
        bot_member = await bot.get_chat_member(chat_id, bot_info.id)
        missing_rights = missing_registration_rights(bot_member)
        if missing_rights:
            rights_text = "\n".join(f"• {html.escape(right)}" for right in missing_rights)
            await callback.message.edit_text(
                "❌ <b>Регистрация не завершена</b>\n\n"
                "AniGuard не получил все права, необходимые для модерации.\n\n"
                "<b>Отсутствующие права:</b>\n" + rights_text + "\n\n"
                "Откройте настройки группы → Администраторы → AniGuard, выдайте указанные права и нажмите кнопку ещё раз.",
                reply_markup=registration_keyboard(chat_id),
            )
            await callback.answer("Не хватает прав администратора.", show_alert=True)
            return

        async with SessionFactory() as session:
            registered_chat = await sync_chat_from_telegram(session, bot, chat_id)
            registered_chat.is_active = True
            registered_settings = dict(registered_chat.settings or {})
            registered_settings.update({
                "registration_completed": True,
                "registered_by": callback.from_user.id,
                "registered_at": utcnow().isoformat(),
            })
            registered_chat.settings = registered_settings
            await session.commit()

        success_keyboard = None
        if bot_info.username:
            success_keyboard = InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="🚀 Открыть AniGuard", url=f"https://t.me/{bot_info.username}?start=panel")
            ]])
        await callback.message.edit_text(
            "✅ <b>Беседа зарегистрирована</b>\n\n"
            "AniGuard получил необходимые права администратора.\n\n"
            "🛡 Защита и модерация запущены.\n"
            "⚙️ Настройки доступны в Mini App.\n"
            "📋 Для открытия панели используйте команду /panel.",
            reply_markup=success_keyboard,
        )
        await callback.answer("Беседа зарегистрирована. AniGuard начал работу.")
    except Exception as exc:
        logger.exception("Could not register chat %s: %s", chat_id, exc)
        try:
            await callback.answer(f"Не удалось зарегистрировать беседу: {str(exc)[:120]}", show_alert=True)
        except Exception:
            pass
...
@router.message(F.left_chat_member | F.pinned_message)
async def cleanup_membership_service_message(message: Message) -> None:
    """Delete Telegram service messages without deleting the pinned content itself."""
    try:
        await message.delete()
    except Exception as exc:
        logger.debug(
            "Не удалось удалить служебное сообщение: chat_id=%s message_id=%s error=%s",
...
async def enforce_message_protection(message: Message, chat: Chat, membership: Membership) -> bool:
    async with SessionFactory() as session:
        ...
        # Администраторы пропускают обычную автомодерацию, но запрещённое
        # визуальное содержимое удаляется для всех участников без исключения.
        if privileged_sender:
            if local_media_safety_trigger:
                applied = await apply_automatic_violation(
                    "local_unsafe_media",
                    local_media_safety_reason or "Обнаружен запрещённый визуальный контент",
                    add_warning=False,
                    quarantine=False,
                )
                try:
                    await bot.send_message(
                        chat.id,
                        "🚫 <b>Медиа удалено</b>\n\n"
                        "Локальная проверка обнаружила откровенный или запрещённый контент.",
                    )
                except Exception:
                    logger.exception("Could not send local media safety notice to chat %s", chat.id)
                return applied
            return False
...
async def _execute_builtin_special(
    ...
    if special == "pin_reply":
        ...
        await bot.pin_chat_message(chat_id=chat.id, message_id=message.reply_to_message.message_id, disable_notification=True)
        return True, "Сообщение закреплено."
    if special == "unpin_reply":
        if not message.reply_to_message:
            raise ValueError("Ответьте командой «открепить» на закреплённое сообщение")
        await bot.unpin_chat_message(chat_id=chat.id, message_id=message.reply_to_message.message_id)
...
async def try_basic_moderation_command(message: Message, chat: Chat, membership: Membership) -> bool:
    ...
    async with SessionFactory() as session:
        ...
        if special_handled:
            await session.commit()
            if special_result:
                special_key = str(config.get("special") or "").strip()
                normalized_result = _normalize_phrase(special_result)
                direct_response = None
                if special_key == "pin_reply" or normalized_result == "сообщение закреплено":
                    direct_response = "Сообщение закреплено"
                elif special_key == "unpin_reply" or normalized_result == "сообщение откреплено":
                    direct_response = "Сообщение откреплено"
                elif (
                    special_key in {"unpin_all", "unpinall"}
                    or normalized_result in {
                        "все сообщения откреплены",
                        "все закрепленные сообщения сняты",
                        "все закрепы сняты",
                    }
                ):
                    direct_response = "Все сообщения откреплены"

                if direct_response and False:
                    await message.answer(direct_response)
                elif style != "custom" and config.get("_response_overridden") and response:
                    await _send_compact_command_result(
                        message, command=config, target_id=target_id,
                        reason=special_result, chat_title=chat.title,
                    )
                else:
                    special_response = render_action_response(
                        style=style,
                    ...
                    if special_title:
                        special_response = special_response.replace(
                            "Открытие дела выполнено", special_title, 1
                        )
...
# Unified chat-info aliases must run before the broad group pipeline.
@router.message(
...
async def unified_chatinfo_alias_handler(message: Message) -> None:
